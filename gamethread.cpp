...
void Game::goFixedDepth() {
	...
	for(; max_depth <= max_depth_global; ++max_depth) {
		...
		negamax(game_board, -INFINITY, INFINITY, max_depth, 0, FIXED_DEPTH, false, false, new PV());
		hasBestMove = true;
...
void Game::goFixedTime(int tm) {
	...
	for(max_depth = 1; timer.getTime() < time; ++max_depth) {
		...
		negamax(game_board, -INFINITY, INFINITY, max_depth, 0, FIXED_TIME, false, false, new PV());
